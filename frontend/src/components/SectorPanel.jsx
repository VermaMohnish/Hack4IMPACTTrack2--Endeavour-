...
const SectorPanel = () => {
  ...
  // FIX 3: Track which direction the dropdown should open for each button
  const [dropdownDir, setDropdownDir] = useState("down"); // "down" | "up"
  const buttonRefs = useRef({});
  const scrollContainerRef = useRef(null);

  // FIX 3: When opening, check if there's enough room below; if not, open upward
  const handleToggle = (sectorId) => {
    ...
    const btn = buttonRefs.current[sectorId];
    const container = scrollContainerRef.current;
    if (btn && container) {
      const btnRect = btn.getBoundingClientRect();
      const containerRect = container.getBoundingClientRect();
      // Dropdown height is approx 4 items × ~42px + header ~36px ≈ 204px
  ...
  useEffect(() => {
    const handleClick = (e) => {
      // Check if click is outside ALL button refs and their dropdowns
      const isInsideAny = Object.values(buttonRefs.current).some(
        (ref) => ref && ref.closest(".command-wrapper")?.contains(e.target),
      );
      if (!isInsideAny) {
        setOpenSectorId(null);
      }
  ...
  return (
    <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 shadow-2xl h-full flex flex-col font-exo relative overflow-hidden">
      ...
      <div className="flex justify-between items-end mb-4 border-b border-slate-700/50 pb-3 relative z-10 flex-shrink-0">
        <div>
        ...
        <div className="flex flex-col items-end">
          <span className="text-[10px] uppercase font-share text-slate-400 tracking-wider">
            Active Fleet
          </span>
          <span className="text-lg font-rajdhani font-bold text-emerald-400 leading-none">
            {totalAvailable} Ready
          </span>
        </div>
      </div>

      {/* FIX 3: ref on scroll container so we can measure space below each button */}
      <div
        ref={scrollContainerRef}
        className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar relative z-10 p-1 min-h-0"
      >
        {sectors.map((sector) => {
          const sectorHasAvailableUnits = assets.some(
            (a) => a.sectorId === sector.id && a.status === "AVAILABLE",
          );
            ...
            >
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.5)] ${sector.risk === "High" ? "bg-red-500 shadow-red-500 animate-pulse" : sector.risk === "Warning" ? "bg-yellow-500 shadow-yellow-500 animate-pulse" : "bg-emerald-500 shadow-emerald-500"}`}
                  ></div>
                  <h3 className="font-bold text-white text-md font-rajdhani tracking-wide">
                    {sector.name}
                  </h3>
                  <span className="text-[10px] text-slate-500 font-share font-bold ml-1">
                    SEC-0{sector.id}
                  </span>
                </div>

                {/* FIX 3: "command-wrapper" class used for outside-click detection */}
                <div className="command-wrapper relative">
                  <button
                    ref={(el) => (buttonRefs.current[sector.id] = el)}
                  ...
                  >
                    <Send className="w-3 h-3 mr-1.5 group-hover:translate-x-0.5 transition-transform" />{" "}
                    COMMAND
                  </button>

                  {/* FIX 3: Conditionally place dropdown above or below the button */}
                  {openSectorId === sector.id && (
                    <div
                      className={`absolute right-0 w-44 bg-slate-900 border border-slate-700/70 rounded-xl shadow-2xl z-[9999] overflow-hidden
                        ${dropdownDir === "up" ? "bottom-full mb-2" : "top-full mt-2"}`}
                    >
                      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          Dispatch to {sector.name}
                        </span>
                        <button
                          onClick={() => setOpenSectorId(null)}
                      ...
                      {ASSET_TYPES.map(({ type, label, icon: Icon, color }) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${available > 0 ? "text-emerald-400 bg-emerald-950/40" : "text-slate-500 bg-slate-800"}`}
                            >
                              {available} avail
                            </span>
                          </button>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex-1 flex items-center justify-center gap-1.5 bg-slate-900/60 border border-slate-700/50 py-1.5 rounded-lg text-[10px] sm:text-xs text-slate-300 shadow-inner">
                    <PlusSquare className="w-3 h-3 text-rose-400" />{" "}
                    {sector.hospitals}
                  </div>
                  <div className="flex-1 flex items-center justify-center gap-1.5 bg-slate-900/60 border border-slate-700/50 py-1.5 rounded-lg text-[10px] sm:text-xs text-slate-300 shadow-inner">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 flex items-center justify-center gap-1 bg-blue-950/20 border border-blue-900/30 py-1 rounded-lg text-[9px] sm:text-xs text-slate-400 font-share">
                    <Shield className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-blue-500/70" />
                    <span className="text-blue-400">
                      {getAvailableCount(sector.id, "PoliceJeep")}
                    </span>{" "}
                    Jeep
                  </div>
                  <div className="flex-1 flex items-center justify-center gap-1 bg-emerald-950/20 border border-emerald-900/30 py-1 rounded-lg text-[9px] sm:text-xs text-slate-400 font-share">
                    <Navigation className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-emerald-500/70" />
                    <span className="text-emerald-400">
                      {getAvailableCount(sector.id, "Helicopter")}
                    </span>{" "}
                    Heli
                  </div>
                  <div className="flex-1 flex items-center justify-center gap-1 bg-rose-950/20 border border-rose-900/30 py-1 rounded-lg text-[9px] sm:text-xs text-slate-400 font-share">
                    <PlusSquare className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-rose-500/70" />
                    <span className="text-rose-400">
                      {getAvailableCount(sector.id, "Ambulance")}
                    </span>{" "}
                    Amb
                  </div>
                  <div className="flex-1 flex items-center justify-center gap-1 bg-orange-950/20 border border-orange-900/30 py-1 rounded-lg text-[9px] sm:text-xs text-slate-400 font-share">
                    <Truck className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-orange-500/70" />
                    <span className="text-orange-400">
                      {getAvailableCount(sector.id, "FireTruck")}
                    </span>{" "}
                    Fire
                  </div>
                </div>
